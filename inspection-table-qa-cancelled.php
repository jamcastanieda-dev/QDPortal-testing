...
$resNotViewed = $conn->query("SELECT inspection_no FROM inspection_cancelled_notification");
if ($resNotViewed) {
...
// Grab filters from POST (or default to empty)
...
if ($request !== '') {
    switch ($request) {
        ...
        default:
            // any other exact-match on the original request column
            $where[]  = 'ir.request = ?';
            $params[] = $request;
            $types   .= 's';
            break;
...
// Final ORDER BY (you can tweak priority if needed)
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Shorten the requestor name (First + initial)
        ...
        // Determine the human label for the request type
        ...
        echo '<td class="status-cancelled">'
