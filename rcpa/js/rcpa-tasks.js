(function () {
  ...
  qmsOpenBtn.addEventListener('click', openQmsModal);
...
(function () {
  ...
  function isQmsFromWindow() {
    try { return ['QMS', 'QA'].includes(String(window.RCPA_DEPARTMENT || '').trim().toUpperCase()); }
    catch { return false; }
  }
  ...
  async function refreshTaskBadges() {
    ...
    try {
      ...
      if (res.ok && data && data.ok && data.counts) {
        const isQms = typeof data.is_qms === 'boolean' ? data.is_qms : isQmsFromWindow();

        // QMS top-left badge (only if QA/QMS)
        if (bQms) {
          if (isQms) setBadge(bQms, data.counts.qms);
          else { bQms.hidden = true; bQms.textContent = '0'; bQms.removeAttribute('title'); bQms.removeAttribute('aria-label'); }
        }
        ...
        if (bQmsApproval) {
          if (isQms) {
            setBadge(bQmsApproval, qmsApprovalTotal);
          } else {
            bQmsApproval.hidden = true;
            bQmsApproval.textContent = '0';
            bQmsApproval.removeAttribute('title');
            bQmsApproval.removeAttribute('aria-label');
          }
        }

        // Modal chips: only show counts for QMS/QA (but keep clickable for everyone)
        setBadge(bQmsReply, isQms ? data.counts.qms_reply_approval : 0);
        setBadge(bQmsClosing, isQms ? data.counts.qms_closing_approval : 0);

        // CLOSED
        setBadge(bClosed, data.counts.closed);

        // NOTE: removed the line that disabled the QMS button for non-QA/QMS users
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const bQms = document.getElementById('badgeQms');
    if (bQms && !isQmsFromWindow()) bQms.hidden = true;
    refreshTaskBadges();
  });
