(function () {
  ...
  async function load() {
    ...
    let res;
    try {
      res = await fetch('../php-backend/rcpa-list-approval-validation-reply.php?' + params.toString(), { credentials: 'same-origin' });
    } catch {
...
(function () {
  ...
  function clearViewForm() {
    ...
    // Reset the "For ..." toggles (robust: duplicate ids/selectors)
    (function setCheckedByAny(selectors, on) {
      const seen = new Set();
      selectors.forEach(sel => {
        modal.querySelectorAll(sel).forEach(cb => {
    ...
    })(['#rcpa-view-for-nc-valid', '#rcpa-view-for-nc', '.checkbox-for-non-conformance input[type="checkbox"]'], false);

    (function setCheckedByAny2(selectors, on) {
      const seen = new Set();
      selectors.forEach(sel => {
        modal.querySelectorAll(sel).forEach(cb => {
          if (cb && !seen.has(cb)) { cb.checked = !!on; seen.add(cb); }
    ...
    })(['#rcpa-view-for-pnc', '.checkbox-for-potential-non-conformance input[type="checkbox"]'], false);

    // Show both “For …” toggles by default on reset
    ['.checkbox-for-non-conformance', '.checkbox-for-potential-non-conformance'].forEach(sel => {
      modal.querySelectorAll(sel).forEach(el => el.removeAttribute('hidden'));
    });

    // Hide both NC/PNC panels
    (function toggleValidPanelsOff() {
      modal.querySelectorAll('.correction-grid:not(.preventive-grid)').forEach(g => { g.style.display = 'none'; });
      const pnc = modal.querySelector('.preventive-grid'); if (pnc) pnc.style.display = 'none';
    })();
    ...
    // Reset disapproval remarks table AND hide its fieldset by default
    const tb = document.querySelector('#rcpa-rejects-table tbody');
  ...
  document.addEventListener('rcpa:action', async (e) => {
    ...
    if (!window.Swal) {
      const ok = confirm('Approve this RCPA validation reply approval? This will set the status to "VALIDATION REPLY APPROVAL".');
      if (!ok) return;
      try {
        const fd = new FormData();
        fd.append('id', id);
        const res = await fetch('../php-backend/rcpa-accept-approval-validation-reply.php', {
          method: 'POST',
          body: fd,
        ...
        if (statusEl) statusEl.value = 'VALIDATION REPLY APPROVAL';
        alert('RCPA approved. Status set to "VALIDATION REPLY APPROVAL".');
      } catch (err) {
        console.error(err);
    ...
    // SweetAlert flow
    Swal.fire({
      title: 'Approve this RCPA validation reply approval?',
      ...
      // reflect in UI
      const statusEl = document.getElementById('rcpa-view-status');
      if (statusEl) statusEl.value = 'FOR CLOSING';
      // refresh the table
      Swal.fire({ icon: 'success', title: 'RCPA approved', text: 'Status set to "ASSIGNEE CORRECTIVEL".', timer: 1600, showConfirmButton: false });
  ...
  rjForm?.addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      const res = await fetch('../php-backend/rcpa-reject-approval-validation-reply.php', {
        method: 'POST',
        body: fd,
        credentials: 'same-origin'
      });
      ...
      if (st) st.value = 'VALIDATION REPLY';

      if (window.Swal) {
        Swal.fire({ icon: 'success', title: 'Returned to Assignee', text: 'Status set to "VALIDATION REPLY".', timer: 1600, showConfirmButton: false });
      } else {
        alert('Returned to VALIDATION REPLY. Status set to "VALIDATION REPLY".');
      }
