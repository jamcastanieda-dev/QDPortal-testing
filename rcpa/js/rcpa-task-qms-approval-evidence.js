...
(function () {
  ...
  @media (max-width:720px){ #rcpa-why-view-list.two-col { grid-template-columns:1fr; } }

  /* base card */
  #rcpa-why-view-modal .why-item{
    padding:10px;
...
`;
    ...
    function clearViewForm() {
        ...
        const fsEv = document.getElementById('rcpa-view-evidence');
        if (fsEv) fsEv.hidden = true;

        // Disapproval list reset + HIDE the fieldset by default
        const tb = document.querySelector('#rcpa-rejects-table tbody');
        ...
        // id bookkeeping
        currentViewId = null;
        document.getElementById('rcpa-view-modal')?.removeAttribute('data-rcpa-id');
    }
    ...
    function ensureWhyWhyButton() {
        ...
        const anchor = modal.querySelector('label.inline-check input#rcpa-view-findings-valid')?.closest('label.inline-check');
        if (!anchor) return;
    ...
    function renderWhyViewList(rows) {
        ...
        const groups = rows.reduce((m, r) => ((m[r.analysis_type] = m[r.analysis_type] || []).push(r), m), {});
        const entries = Object.entries(groups);
        if (entries.length === 2) whyViewList.classList.add('two-col'); else whyViewList.classList.remove('two-col');

        const html = entries.map(([type, arr]) => {
            const items = arr.map((r, i) => {
                const isRoot = ['yes', 'true', '1', 'root', 'y'].includes(
                    String(r.answer ?? r.is_root ?? r.root ?? '').trim().toLowerCase()
                );
                return `
    <div class="why-item${isRoot ? ' is-root' : ''}">
      <div style="font-weight:600;margin-bottom:6px;">Why ${i + 1}${isRoot ? ` <span style="padding:2px 6px;border-radius:999px;border:1px solid #16a34a;font-size:12px;">Root Cause</span>` : ''
                    }</div>
    ...
    async function fillViewModal(row) {
        ...
        // Persist id for other helpers and Why-Why
        document.getElementById('rcpa-view-modal')?.setAttribute('data-rcpa-id', String(row.id || ''));
        bindWhyWhyButton();
    }
    ...
    // Helper to resolve the active record id safely
    function getActiveRcpaId(evtDetail) {
        const fromEvent = evtDetail?.id;
        const fromState = currentViewId;
        const fromDataset = Number(document.getElementById('rcpa-view-modal')?.dataset?.rcpaId || 0);
    ...
    // REJECT button just opens the modal (keep yours)
    document.addEventListener('rcpa:action', (e) => {
        const { action } = e.detail || {};
        if (action !== 'reject') return;
        currentViewId = getActiveRcpaId(e.detail); // remember id for submit
        openRejectModal();
    });
    ...
    rjForm?.addEventListener('submit', async (ev) => {
        ...
        try {
            ...
            const res = await fetch('../php-backend/rcpa-reject-evidence-approval.php', {
                method: 'POST',
                credentials: 'same-origin',
                body: fd
            });
            ...
            if (rejectModal) { rejectModal.classList.remove('show'); rejectModal.setAttribute('hidden', ''); }
            const view = document.getElementById('rcpa-view-modal');
            if (view && !view.hidden) { view.classList.remove('show'); view.setAttribute('hidden', ''); }
            document.body.style.overflow = '';
