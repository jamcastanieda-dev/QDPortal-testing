(function () {
    ...
    function todayYmdManila() {
        const parts = new Intl.DateTimeFormat('en-CA', { timeZone: MANILA_TZ, year: 'numeric', month: '2-digit', day: '2-digit' })
    ...
    function diffDaysFromTodayManila(ymd) {
        ...
        return Math.trunc((target - today) / 86400000);
    }
    ...
    async function load() {
        ...
        let res;
        try {
            res = await fetch('../php-backend/rcpa-list-approval-assignee-corrective.php?' + params.toString(), { credentials: 'same-origin' });
        } catch {
...
(function () {
    ...
    function renderRejectsTable(items) {
        ...
        const fs = document.querySelector('fieldset.reject-remarks, #rcpa-rejects-fieldset');
        if (!tb) return;
    ...
    // ---------- Reset modal ----------
    // ---- RESET (hide rejects fieldset on reset) ----
    function clearViewForm() {
        ...
        [
            ...
            // corrective action evidence (already in your file)
        ...
        // clear attachments (remarks + validation + corrective evidence)
    ...
    document.addEventListener('rcpa:action', async (e) => {
        ...
        if (!window.Swal) {
            const ok = confirm('Approve this RCPA corrective action approval? This will set the status to "EVIDENCE CHECKING".');
            if (!ok) return;
            try {
                const fd = new FormData();
                fd.append('id', id);
                const res = await fetch('../php-backend/rcpa-accept-approval-corrective.php', {
                    method: 'POST',
                    body: fd,
                ...
                if (statusEl) statusEl.value = 'EVIDENCE CHECKING';
                alert('RCPA approved. Status set to "EVIDENCE CHECKING".');
        ...
        // SweetAlert flow
        Swal.fire({
            title: 'Approve this RCPA corrective action approval?',
            text: 'This will set the status to "EVIDENCE CHECKING".',
            icon: 'question',
            ...
            if (statusEl) statusEl.value = 'EVIDENCE CHECKING';
            // refresh the table
            Swal.fire({ icon: 'success', title: 'RCPA approved', text: 'Status set to "EVIDENCE CHECKING".', timer: 1600, showConfirmButton: false });
            document.dispatchEvent(new CustomEvent('rcpa:refresh'));
        });
    });
    ...
    rjForm?.addEventListener('submit', async (ev) => {
        ...
        try {
            ...
            const res = await fetch('../php-backend/rcpa-reject-approval-corrective.php', {
                method: 'POST',
                body: fd,
                credentials: 'same-origin'
            });
            ...
            if (window.Swal) {
                Swal.fire({ icon: 'success', title: 'Returned to Assignee', text: 'Status set to "FOR CLOSING".', timer: 1600, showConfirmButton: false });
            } else {
                alert('Returned to FOR CLOSING. Status set to "FOR CLOSING".');
            }
