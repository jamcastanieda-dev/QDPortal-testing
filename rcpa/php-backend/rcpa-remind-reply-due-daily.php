...
// logmsg('--- RCPA reply-due reminder job started ---');
...
foreach ($rows as $r) {
  ...
  $activityStr = 'Reminder: Reply due in ' . $label; // e.g., "Reminder: Reply due in 2 days" or "today"
  ...
  // Compose email (improved design + requested link changes)
  $deptDisplay      = $dept . ($section !== '' ? ' - ' . $section : '');
  $subject          = ($daysLeft === 0)
    ? sprintf('RCPA #%d - Reply due today (%s)', (int)$id, $deptDisplay)
    : sprintf('RCPA #%d - %s left to reply (%s)', (int)$id, $label, $deptDisplay);

  // Task URL (button target and fallback link). Use http:// to ensure clickability in email clients.
...
<body style="margin:0; padding:0; background:#f3f4f6; font-family:Arial,Helvetica,sans-serif; color:#111827;">
          ...
          <tr>
            <td style="padding:20px;">
              ...
              <p style="margin:0 0 18px 0; font-size:14px;">
                This is a friendly reminder that the reply for <strong>RCPA #'.(int)$id.'</strong> is
  ...
  $altBody = ($daysLeft === 0)
    ? "RCPA #$id - Reply due today\n"
    : "RCPA #$id - Reply due in $label\n";
