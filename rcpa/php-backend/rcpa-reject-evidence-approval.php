...
try {
  ...
  // Must currently be in EVIDENCE APPROVAL
  $stmt = $conn->prepare("SELECT id FROM rcpa_request WHERE id=? AND status='EVIDENCE APPROVAL'");
  ...
  if (!empty($_FILES['attachments'])) {
    ...
    $relPath = "uploads-rcpa-evidence-approval-reject/$stamp/";
    ...
    for ($i=0; $i<$count; $i++) {
      ...
      $filesOut[] = ['name'=>$filename, 'url'=>$webBase.$filename, 'size'=>$size, 'mime'=>$mime];
    }
  ...
  // Store a disapproval record (if you track them)
  if ($attachmentsJson !== null) {
    $type = 'Evidence approval rejected';
    $insR = $conn->prepare("INSERT INTO rcpa_disapprove_remarks (rcpa_no, disapprove_type, remarks, attachments) VALUES (?,?,?,?)");
  ...
  // History
  $activity = 'Returned to originator for more evidence';
