<?php
// rcpa-list-approval-invalidation-reply.php
...
/* Visibility restriction:
   ...
   - Else:
       * If department known: (assignee = ? OR originator_name = ?)
       * If department unknown: only originator_name = ?
*/
if (!$isQaqms) {
    if ($dept !== '') {
        ...
        if ($user_name !== '') {
            ...
            // extremely defensive: no identity info -> show nothing
            $where[] = "1=0";
        }
    }
}
...
while ($r = $res->fetch_assoc()) {
    $rows[] = [
        ...
        'assignee_name'    => (string)($r['assignee_name'] ?? ''),  // Include assignee_name
    ];
}
