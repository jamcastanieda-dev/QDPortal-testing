...
function evt($d){ echo "event: rcpa-notif-approval-count\n"; echo 'data: '.json_encode($d,JSON_UNESCAPED_UNICODE)."\n\n"; @flush(); }
...
try{
  ...
  $getCount = function() use($mysqli,$user_name){
    $c=0;
    $sql="SELECT COUNT(*) FROM rcpa_request WHERE status IN ('FOR APPROVAL OF MANAGER') AND LOWER(TRIM(originator_supervisor_head))=LOWER(TRIM(?))";
