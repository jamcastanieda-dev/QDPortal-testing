...
try {
  if (!isset($_COOKIE['user'])) {
    ...
    send_evt('rcpa-notif-tasks-count', ['ok' => false, 'error' => 'Not authenticated']);
    exit;
  }
  $user = json_decode($_COOKIE['user'], true);
  if (!$user || !is_array($user)) {
    http_response_code(401);
    send_evt('rcpa-notif-tasks-count', ['ok' => false, 'error' => 'Invalid user']);
    exit;
  }
  ...
  if (!$mysqli || $mysqli->connect_errno) {
    ...
    send_evt('rcpa-notif-tasks-count', ['ok' => false, 'error' => 'DB unavailable']);
    exit;
  }
  ...
  // Status lists
  $status_list_all = "(
    'QMS CHECKING','VALIDATION REPLY','INVALIDATION REPLY','EVIDENCE CHECKING',
    'ASSIGNEE PENDING','FOR CLOSING',
    'VALID APPROVAL','INVALID APPROVAL','FOR CLOSING APPROVAL',
    'INVALIDATION REPLY APPROVAL','EVIDENCE APPROVAL',
    'CLOSED (VALID)','CLOSED (INVALID)'
  )";
