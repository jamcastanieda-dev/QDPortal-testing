...
/* ---------------------------
   ...
   (If user isn't in system_users, return empty)
--------------------------- */
$dept = '';
if ($user_name !== '') {
...
$page_size_param = min(100, max(1, (int)($_GET['page_size'] ?? 10)));
if ($dept === '') {
    echo json_encode([
        'page'      => 1,
...
/* QA/QMS can see all rows; others are restricted to their department */
$dept_norm = strtolower(trim($dept));
$isQms = ($dept_norm === 'qms');
...
/* ---------------------------
   WHERE builder
--------------------------- */
$allowed_statuses = ['CLOSING APPROVAL'];

$where  = [];
$params = [];
$types  = '';

/* Restrict by assignee ONLY if not QA/QMS */
if (!$isQms) {
    $where[]  = "assignee = ?";
    $params[] = $dept;
    $types   .= 's';
...
if ($types !== '') { // avoid bind_param with no placeholders
