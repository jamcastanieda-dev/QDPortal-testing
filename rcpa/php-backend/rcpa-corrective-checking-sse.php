<?php
// php-backend/rcpa-corrective-checking-sse.php
...
// drain any output buffers and enable implicit flush
...
/* -------- Resolve user's department + QA/QMS flag -------- */
$user_dept = null;
$is_qaqms  = false;
...
/* -------- Inputs (mirror your list filter) -------- */
$type = trim((string)($_GET['type'] ?? '')); // optional rcpa_type filter

/* -------- WHERE (mirror rcpa-list-corrective-checking.php) -------- */
$allowed_statuses = ['EVIDENCE CHECKING'];
...
/* Visibility:
   - QA/QMS: see all
   - Others: (assignee = user's dept OR originator_name = user_name)
*/
if (!$is_qaqms) {
  if (!empty($user_dept)) {
...
/* -------- Snapshot we watch: MAX(id) + COUNT(*) over the same WHERE -------- */
$sql = "SELECT MAX(id) AS max_id, COUNT(*) AS cnt
        FROM rcpa_request
        WHERE $where_sql";

$deadline  = time() + 60;   // ~60s per connection; EventSource auto-reconnects
$lastPing  = 0;
$lastMaxId = 0;
$lastCount = 0;
