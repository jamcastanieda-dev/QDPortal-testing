...
const $viewStatus = document.getElementById('filter-view-status');
...
function loadInspections() {
    ...
    if ($viewStatus.value) params.append('view_status', $viewStatus.value); // <-- Add this

    fetch('inspection-table-qa-completed.php', {
        method: 'POST',
...
$viewStatus.addEventListener('change', loadInspections);
...
function connectWebSocket() {
    ...
    socket.onmessage = function (event) {
        ...
        try {
            ...
            if (message.action === 'approved' || message.action === 'viewed') {
                loadInspections();
            }
        } catch (error) {
