...
// 2) Build WHERE clauses (always include company = 'ssd')
$where = [];
$params = [];
$types = '';
...
if ($date !== '') {
  $where[] = "(
    -- PAINTING: Only match if painting is not empty and status is allowed
    (COALESCE(iio.painting, '') <> ''
      AND ir.status IN ('COMPLETED', 'PASSED', 'FAILED', 'PASSED W/ FAILED', 'FOR RESCHEDULE')
      AND STR_TO_DATE(LEFT(ir.date_time, 10), '%d-%m-%Y') = ?)
    OR
    -- NON-PAINTING: Only match if status is allowed
