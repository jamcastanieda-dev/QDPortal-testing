...
function populateInspectionRequest(data, inspectionNo) {
  ...
  if (data.request === "Final & Sub-Assembly Inspection") {
    ...
    // ── Painting under Outgoing special case ─────────────────────────
    if (data.painting === "y") {
      document.getElementById("view-outgoing-inspection").checked = true;
      document.querySelector(".nested-outgoing-options").style.display = "block";
...
function fetchIncomingOutgoingRequest(inspectionNo) {
  ...
  fetch("inspection-request-retrieve-incoming.php", {
    ...
    .then((data) => {
      if (data.status === "success") {
        console.log('populateIncomingOutgoingRequest got record:', data.data);
        console.log('→ painting flag:', data.data.painting);
...
function populateIncomingOutgoingRequest(data, inspectionNo) {
  // ── Reset Painting sub-group ───────────────────────────────
  const nestedOpts = document.querySelector(
    "#viewInspectionModal .nested-outgoing-options"
  );
  if (nestedOpts) nestedOpts.style.display = "none";
  const viewPainting = document.getElementById("view-painting-inspection");
  if (viewPainting) viewPainting.checked = false;
  ...
  if (data.painting === "y") {
    // ensure Outgoing is selected
    document.getElementById("view-outgoing-inspection").checked = true;
    // reveal and check Painting
    if (nestedOpts) nestedOpts.style.display = "block";
    if (viewPainting) viewPainting.checked = true;
  }
}
...
function fetchInspectionCompletedAttachments(inspectionNo) {
  ...
  fetch("inspection-request-retrieve-completed-attachments.php", {
    ...
    .then((data) => {
      const attachmentsContainer = document.getElementById("view_completed-attachments");
      attachmentsContainer.innerHTML = ""; // Clear previous attachments

      if (data.status === "no_attachment") {
        // Hide the attachments section if not needed
        attachmentsContainer.style.display = "none";
        // Optionally show a message elsewhere if you want
        // document.getElementById("some-message-div").textContent = data.message;
        return;
      } else {
        // Always show the container if attachments may be present
      ...
      if (data.status === "success") {
        if (data.files.length === 0) {
          ...
          document.getElementById("completed-document-no").value = data.document_no;
          document.getElementById("completed-by").textContent = "Completed By: " + data.completed_by;
      ...
      if (data.status == "success") {
        if (data.incoming.length > 0) {
          ...
          for (let i = 0; i < data.incoming.length; i++) {
            ...
            const baseQty = (item.quantity ?? '').toString();
            const baseQtyDisp = baseQty.trim() !== '' ? baseQty : '–';
            ...
            if (
              ...
              // Initiator + REJECTED/CANCELLED
              newRow.innerHTML = `
                              <div class="wbs-group">
                                <input type="text" id="view-wbs-${row}" value="${item.wbs}" readonly>
                              </div>
                              <div class="description-group">
                                <input type="text" id="view-desc-${row}" value="${item.description}" readonly>
                              </div>
                              <div class="wbs-quantity-group">
                                <input type="text" id="view-qty-${row}" value="${baseQtyDisp}" readonly>
                              </div>
                            `;
            } else {
              ...
              if (
                ...
                if (item.status === "passed") {
                  newRow.innerHTML = `
                                  <div class="wbs-group">
                                    <input type="text" id="view-wbs-${row}" value="${item.wbs}" class="pass-wbs" readonly>
                                  </div>
                                  <div class="description-group">
                                    <input type="text" id="view-desc-${row}" value="${item.description}" class="pass-wbs" readonly>
                                  </div>
                                  <div class="wbs-quantity-group">
                                    <div class="qty-col">
                                      <label for="view-req-qty-${row}">Requested:</label>
                                      <input type="text" id="view-req-qty-${row}" value="${baseQtyDisp}" class="pass-wbs" readonly>
                                    </div>
                                    <div class="qty-col">
                                      <label for="view-passed-qty-${row}">Passed:</label>
                                      <input type="text" id="view-passed-qty-${row}" value="${passDisplay}" class="pass-wbs" readonly>
                                    </div>
                                  </div>
                                `;
                  ...
                  newRow.innerHTML = `
                                  <div class="wbs-group">
                                    <input type="text" id="view-wbs-${row}" value="${item.wbs}" class="fail-wbs" readonly>
                                  </div>
                                  <div class="description-group">
                                    <input type="text" id="view-desc-${row}" value="${item.description}" class="fail-wbs" readonly>
                                  </div>
                                  <div class="wbs-quantity-group">
                                    <div class="qty-col">
                                      <label for="view-req-qty-${row}">Requested:</label>
                                      <input type="text" id="view-req-qty-${row}" value="${baseQtyDisp}" class="fail-wbs" readonly>
                                    </div>
                                    <div class="qty-col">
                                      <label for="view-failed-qty-${row}">Failed:</label>
                                      <input type="text" id="view-failed-qty-${row}" value="${failDisplay}" class="fail-wbs" readonly>
                                    </div>
                                  </div>
                                `;
                  ...
                  newRow.innerHTML = `
                                      <div class="wbs-group">
                                        <label>&nbsp;</label>
                                        <input type="text" id="view-wbs-${row}" value="${item.wbs}" class="pwf-wbs" readonly>
                                      </div>
                                      <div class="description-group">
                                        <label>&nbsp;</label>
                                        <input type="text" id="view-desc-${row}" value="${item.description}" class="pwf-wbs" readonly>
                                      </div>
                                      <div class="wbs-quantity-group">
                                        <div class="qty-col">
                                          <label for="view-req-qty-${row}">Requested:</label>
                                          <input type="text" id="view-req-qty-${row}" value="${baseQtyDisp}" class="pwf-wbs" readonly>
                                        </div>
                                        <div class="qty-col">
                                          <label for="view-passed-qty-${row}">Passed:</label>
                                          <input type="text" id="view-passed-qty-${row}" value="${pwfPassDisp}" class="pwf-wbs" readonly>
                                        </div>
                                        <div class="qty-col">
                                          <label for="view-failed-qty-${row}">Failed:</label>
                                          <input type="text" id="view-failed-qty-${row}" value="${pwfFailDisp}" class="pwf-wbs" readonly>
                                        </div>
                                      </div>
                                    `;
