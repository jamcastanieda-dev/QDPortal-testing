...
// Redirect QA-Head-Inspection to a different page
if (isset($user['privilege']) && $user['privilege'] == 'QA-Head-Inspection') {
    header('Location: inspection-tasks-qa-head.php');
    exit;
}
// Only allow QA-Respondent to proceed
if (!isset($user['privilege']) || $user['privilege'] != 'QA-Respondent') {
    header('Location: login.php');
    exit;
}
