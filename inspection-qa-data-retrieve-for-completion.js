...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchInspectionData() {
        ...
        const actionContainer = document.getElementById('action-container-completion');
        ...
        document.getElementById('view-completion-button').addEventListener('click', function () {
            fetchInspectionRequest(inspectionNo);
            openModal();
            actionContainer.classList.add('hidden');
            if (currentTarget) {
                        ...
                        .then(result => {
                            if (result.success) {
                                Swal.fire({
                                    ...
                                    DateTime();
                                    InsertRequestHistory(inspectionNo, formattedTime, 'Completion of inspection request has been approved.');
                                    InsertRequestHistory(inspectionNo, formattedTime, 'Inspection request has passed.');
        ...
        document.getElementById('reject-completion-button').addEventListener('click', function () {
            showRejectModal();
            actionContainer.classList.add('hidden');
            if (currentTarget) {
                const icon = currentTarget.querySelector('i');
                        ...
                        .then(result => {
                            if (result.success) {
                                Swal.fire({
                                    ...
                                    DateTime();
                                    InsertRequestHistory(inspectionNo, formattedTime, 'Completion for inspection request has been rejected. Reason: ' + rejectRemarks.value);
                                    hideRejectModal();
                                    if (socket && socket.readyState === WebSocket.OPEN) {
                                        socket.send(JSON.stringify({
                                            action: 'change'
        ...
        fetch('inspection-request-retrieve-initiator.php', {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    if (data.data.status == 'PENDING' && data.data.approval == 'FOR COMPLETION') {
