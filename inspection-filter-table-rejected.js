...
const $viewStatus = document.getElementById('filter-view-status'); // Add this line
...
function loadInspections() {
    ...
    if ($wbs && $wbs.value.trim()) params.append('wbs', $wbs.value.trim());
    if ($req && $req.value) params.append('request', $req.value);
    if ($viewStatus && $viewStatus.value !== undefined) params.append('view_status', $viewStatus.value); // Add this line

    fetch('inspection-table-qa-reject.php', {
        method: 'POST',
        ...
        .then(html => {
            ...
            setupPagination(document.querySelector('.inspection-table')); // 🔁 Use your existing function
        })
        .catch(error => {
...
// Debounced typing filter
const loadDebounced = debounce(loadInspections, 300);

// Bind input events
if ($wbs) $wbs.addEventListener('input', loadDebounced);
if ($req) $req.addEventListener('change', loadInspections);
if ($viewStatus) $viewStatus.addEventListener('change', loadInspections); // Add this line
...
function connectWebSocket() {
    ...
    socket.onmessage = function (event) {
        ...
        try {
            ...
            if (message.action == 'change' || message.action == 'request') {
                loadInspections();
            }
        } catch (error) {
