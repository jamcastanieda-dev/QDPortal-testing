...
function loadInspections() {
    ...
    fetch('inspection-table-qa-pending.php', {
        method: 'POST',
        ...
        .then(html => {
            $tbody.innerHTML = html.trim() || '<tr><td colspan="9">No inspections found.</td></tr>';
...
function connectWebSocket() {
    ...
    socket.onmessage = function (event) {
        ...
        try {
            ...
            if (message.action === 'change' || message.action === 'approval' || message.action === 'approved') {
                loadInspections();
            }
        } catch (error) {
