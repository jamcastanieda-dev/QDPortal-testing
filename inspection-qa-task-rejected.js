...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchInspectionData() {
        ...
        `;
            ...
            if (event.target.classList.contains('view-btn')) {
                ...
                // --- NEW: Delete notification row for this inspection number
                fetch('inspection-request-delete-rejected-notification.php', {
                    method: 'POST',
                ...
                fetchRejectedAttachments(inspectionNo); // ✅ ADD THIS LINE
                openModal();
            }
        });
    ...
    function fetchRejectedAttachments(inspectionNo) {
        const formData = new FormData();
        formData.append('inspection_no', inspectionNo);

        fetch('inspection-get-rejected-attachments.php', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
            .then(data => {
                const section = document.getElementById('rejected-attachments-section');
                const list = document.getElementById('rejected-attachments-list');

                // Clear any existing content
                list.innerHTML = '';

                if (data.status === 'success' && data.attachments.length > 0) {
                    section.style.display = 'block'; // ✅ Show section
                    data.attachments.forEach(file => {
                        const li = document.createElement('li');
                        const a = document.createElement('a');
                        a.href = `${file.attachment}`; // Adjust this if needed
                        a.target = '_blank';
                        a.textContent = file.attachment;
                        li.appendChild(a);
                        list.appendChild(li);
                    });
                } else {
            ...
            .catch(error => {
                ...
                document.getElementById('rejected-attachments-section').style.display = 'none'; // fallback hide
            });
    }
